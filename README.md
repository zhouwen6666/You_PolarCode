...
| 模型 | 参数量 | 测试准确率 | Macro F1 | 低 SNR (-4~2dB) | 高 SNR (4~20dB) | 备注 |
|------|--------|-----------|----------|-----------------|------------------|------|
| **PolarMod-FocalG3** | 48,964 | **58.86%** | 57.75% | 15.66% | 78.05% | 总体最优 |
| **PolarMod-WSqrt** | 48,964 | 56.72% | **60.96%** | 14.12% | 75.65% | 类别均衡最优 |
| PolarMod-CE | 48,964 | 52.80% | 50.71% | 12.04% | 70.92% | 基线（消融对照） |
| TDMRNet | — | 51.35% | 50.75% | — | — | 主基线 |
| ResNet | — | 49.12% | 45.63% | — | — | 论文 baseline |
| Inception | — | 32.88% | 28.40% | — | — | 论文 baseline |
| DenseNet | — | 31.41% | 25.91% | — | — | 论文 baseline |

### 消融实验：PolarMod 损失函数对比

对 PolarMod 进行损失函数消融实验，比较标准交叉熵、Focal Loss 和加权交叉熵三种策略。FocalG3 和 WSqrt 变体均从 CE 基线 checkpoint 微调（lr=5e-5, weight_decay=5e-4, grad_clip=1.0）。

| 变体 | 损失函数 | 测试准确率 | Δ vs 基线 | Macro F1 |
|------|----------|-----------|-----------|----------|
| A: PolarMod-CE | CrossEntropyLoss | 52.80% | — | 50.71% |
| B: PolarMod-FocalG3 | Focal Loss γ=3 | 58.86% | **+6.05%** | 57.75% |
| C: PolarMod-WSqrt | Weighted CE (sqrt_inverse_acc) | 56.72% | +3.92% | 60.96% |

**结论**：Focal Loss γ=3 贡献最大（+6.05%），加权 CE 次之（+3.92%），Focal 优于 Weighted +2.14%。Focal 在低/高 SNR 上全面领先。

### 显著性检验

使用 500 次 bootstrap 配对检验，验证 PolarMod 相对各基线的准确率差异是否统计显著。

| 对比 | PolarMod 准确率 | 基线准确率 | 差值 | 95% CI | p 值 | 结论 |
|------|----------------|-----------|------|--------|------|------|
| PolarMod vs TDMRNet | 52.80% | 51.35% | +1.43% | [0.14%, 2.67%] | 0.036 | 显著 |
| PolarMod vs ResNet | 52.80% | 49.12% | +3.73% | [2.34%, 5.12%] | <0.001 | 显著 |
| PolarMod vs DenseNet | 52.80% | 31.41% | +21.39% | [19.78%, 23.01%] | <0.001 | 显著 |
| PolarMod vs Inception | 52.80% | 32.88% | +20.02% | [18.26%, 21.77%] | <0.001 | 显著 |

**结论**：PolarMod 在 4 个基线模型上全部显著优于（p<0.05），95% 置信区间下界均大于 0。
...
├── ablation_study.py       消融实验汇总脚本
├── bootstrap_test.py       Bootstrap 显著性检验脚本
├── run_ablation.sh         一键消融+显著性检验脚本
├── my_polar_llr/           默认数据集
│   ├── train/
│   ├── test/
│   └── metadata.json
└── outputs/                权重、训练历史和测试结果
    ├── ablation_results.json           消融实验汇总
    └── bootstrap_polarmod_vs_*.json    显著性检验结果
```
...
> **小显存注意事项**：RTX 3060 6GB 训练时 `batch_size=64` 可能 OOM，建议降至 `--batch-size 16`。显著性检验推理时 Inception 等大模型需加 `--batch-size 32` 避免 OOM。
...
```powershell
python train.py --model polar_mod --epochs 200 --early-stop-metric acc
```
...
消融实验对 PolarMod 的损失函数进行对比，评估不同损失策略对分类性能的贡献。

### 运行消融汇总

确保三个变体（CE 基线、FocalG3、WSqrt）均已训练并评估完毕，然后运行：

```powershell
python ablation_study.py
```

脚本自动读取各变体的 `test_metrics.json`，生成汇总表和贡献分解，结果保存到 `outputs/ablation_results.json`。

### 消融结果

| 变体 | 损失函数 | 测试准确率 | Δ vs 基线 |
|------|----------|-----------|-----------|
| A: PolarMod-CE | CrossEntropyLoss | 52.80% | — |
| B: PolarMod-FocalG3 | Focal Loss γ=3 | 58.86% | +6.05% |
| C: PolarMod-WSqrt | Weighted CE (sqrt_inverse_acc) | 56.72% | +3.92% |

Focal Loss γ=3 贡献最大（+6.05%），加权 CE 次之（+3.92%），Focal 优于 Weighted +2.14%。

## 显著性检验

使用 500 次 bootstrap 配对重采样，检验 PolarMod 与各基线模型的准确率差异是否统计显著。

### 运行显著性检验

```powershell
# PolarMod vs TDMRNet
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b tdmrnet --ckpt-b outputs/tdmrnet_best.pt `
    --output outputs/bootstrap_polarmod_vs_tdmrnet.json

# PolarMod vs ResNet
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b resnet --ckpt-b outputs/resnet/resnet_best.pt `
    --output outputs/bootstrap_polarmod_vs_resnet.json

# PolarMod vs DenseNet
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b densenet --ckpt-b outputs/densenet/densenet_best.pt `
    --output outputs/bootstrap_polarmod_vs_densenet.json

# PolarMod vs Inception（模型较大，需减小 batch 避免 OOM）
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b inception --ckpt-b outputs/inception/inception_best.pt `
    --output outputs/bootstrap_polarmod_vs_inception.json --batch-size 32
```

`--batch-size` 参数控制推理批大小，默认 64。显存不足时可调小（如 `--batch-size 32`）。

### 检验结果

| 对比 | 差值 | 95% CI | p 值 | 结论 |
|------|------|--------|------|------|
| PolarMod vs TDMRNet | +1.43% | [0.14%, 2.67%] | 0.036 | 显著 |
| PolarMod vs ResNet | +3.73% | [2.34%, 5.12%] | <0.001 | 显著 |
| PolarMod vs DenseNet | +21.39% | [19.78%, 23.01%] | <0.001 | 显著 |
| PolarMod vs Inception | +20.02% | [18.26%, 21.77%] | <0.001 | 显著 |

PolarMod 在全部 4 个基线上均显著优于（p<0.05），95% CI 下界均大于 0。

## 一键脚本

消融汇总和显著性检验也可通过一键脚本完成：

```bash
chmod +x run_ablation.sh
./run_ablation.sh                      # 全流程
./run_ablation.sh --skip-ablation      # 跳过消融，只做显著性检验
./run_ablation.sh --skip-bootstrap     # 跳过显著性检验
```

脚本会自动检查已有 checkpoint 和评估结果，跳过已完成的步骤。
...
# 7. 显著性检验（PolarMod vs 各基线）
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b tdmrnet --ckpt-b outputs/tdmrnet_best.pt --output outputs/bootstrap_polarmod_vs_tdmrnet.json
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b resnet --ckpt-b outputs/resnet/resnet_best.pt --output outputs/bootstrap_polarmod_vs_resnet.json
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b densenet --ckpt-b outputs/densenet/densenet_best.pt --output outputs/bootstrap_polarmod_vs_densenet.json
python bootstrap_test.py --model-a polar_mod --ckpt-a outputs/polar_mod/polar_mod_best.pt `
    --model-b inception --ckpt-b outputs/inception/inception_best.pt --output outputs/bootstrap_polarmod_vs_inception.json --batch-size 32

# 8. 生成可视化图表
python plot_results.py
```

也可以使用一键脚本完成步骤 6-7（消融汇总 + 显著性检验）：
...
- **PolarMod**：基于调制动态通道选择机制，保持 2D 输入和低参数量（48,964 参数），通过 Focal Loss 和类别加权两种损失策略分别微调。
